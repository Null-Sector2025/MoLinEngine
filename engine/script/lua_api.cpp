...
int LuaAPI::Lua_GiveItem(lua_State* L) {
    const char* id = lua_tostring(L, 1); int cnt = lua_tointeger(L, 2); if (cnt <= 0) cnt = 1;
    if (id) GameState::Instance().GetPlayer().inventory[id] += cnt;
    return 0;
}
int LuaAPI::Lua_RemoveItem(lua_State* L) {
    ...
    auto it = inv.find(id);
    if (it != inv.end()) { it->second -= cnt; if (it->second <= 0) inv.erase(it); lua_pushboolean(L, true); }
    else lua_pushboolean(L, false);
    return 1;
}
int LuaAPI::Lua_GetItemCount(lua_State* L) {
    auto& inv = GameState::Instance().GetPlayer().inventory;
    auto it = inv.find(lua_tostring(L, 1));
...
int LuaAPI::Lua_HasItem(lua_State* L) {
    auto& inv = GameState::Instance().GetPlayer().inventory;
    auto it = inv.find(lua_tostring(L, 1));
    int need = lua_tointeger(L, 2); if (need <= 0) need = 1;
...
int LuaAPI::Lua_UseItem(lua_State* L) {
    ...
    if (id) {
        auto proto = RPG::Inventory::GetItemPrototype(id);
        if (proto && proto->onUse) {
            // 需要角色对象，此处简化
            std::cout << "[Lua] Used item: " << id << std::endl;
        }
        Lua_RemoveItem(L);
    }
    return 0;
...
int LuaAPI::Lua_UpdateObjective(lua_State* L) {
    const char* id = lua_tostring(L, 1); int idx = lua_tointeger(L, 2); int prog = lua_tointeger(L, 3);
...
int LuaAPI::Lua_AddExp(lua_State* L) {
    int exp = lua_tointeger(L, 1); auto& p = GameState::Instance().GetPlayer();
    p.exp += exp; if (p.exp >= p.level * 100) { p.exp -= p.level * 100; p.level++; }
    lua_pushinteger(L, p.level); return 1;
}
...
int LuaAPI::Lua_ExecuteBattleCommand(lua_State* L) {
    int idx = lua_tointeger(L, 1); const char* cmd = lua_tostring(L, 2);
    if (g_BattleSystem && cmd) {
        RPG::BattleCommand bc; bc.name = cmd;
        g_BattleSystem->ExecuteCommand(idx, bc);
    }
    return 0;
}
...
// ---- 摄像机 ----
int LuaAPI::Lua_SetCameraPos(lua_State* L) { /* 需要全局摄像机引用 */ return 0; }
...
// ---- HTTP ----
int LuaAPI::Lua_HttpGet(lua_State* L) { std::string r = Network::HttpClient::Get(lua_tostring(L, 1)); lua_pushstring(L, r.c_str()); return 1; }
int LuaAPI::Lua_HttpPost(lua_State* L) { std::string r = Network::HttpClient::Post(lua_tostring(L, 1), lua_tostring(L, 2) ?: ""); lua_pushstring(L, r.c_str()); return 1; }
