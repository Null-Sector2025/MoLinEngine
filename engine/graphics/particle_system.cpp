...
ParticleSystem::ParticleSystem(const std::string& name, int maxParticles)
    : SceneNode(name), m_MaxParticles(maxParticles) {
...
void ParticleSystem::Emit(float x, float y, int count) {
    for (int i = 0; i < count && m_Particles.size() < m_MaxParticles; i++) {
        ...
        p.vy = (rand() % 200 - 300) / 10.0f;
        p.life = 1.0f;
        p.decay = 0.01f + (rand() % 10) / 500.0f;
...
void ParticleSystem::SetColorRange(int rMin, int rMax, int gMin, int gMax, int bMin, int bMax) {
    m_RMin=rMin; m_RMax=rMax; m_GMin=gMin; m_GMax=gMax; m_BMin=bMin; m_BMax=bMax;
}

void ParticleSystem::OnUpdate(float delta) {
    for (auto& p : m_Particles) {
        p.vx += m_GravityX * delta;
        p.vy += m_GravityY * delta;
        p.x += p.vx * delta;
        p.y += p.vy * delta;
        p.life -= p.decay;
    }
...
void ParticleSystem::OnRender(SDL_Renderer* renderer) {
    ...
    for (auto& p : m_Particles) {
        SDL_SetRenderDrawColor(renderer, p.r, p.g, p.b, static_cast<int>(p.a * p.life));
